...
import { COLORS, inputStyle, labelStyle, ghostButton, primaryButton, volumeCardSoftStyle } from '../labStyles';
...
type Props = {
  /** После успешного сохранения (модалка закрывается, страница — toast). */
  onSaved?: () => void;
  showCancel?: boolean;
  onCancel?: () => void;
  /** Компактный заголовок внутри страницы Настройки */
  embedded?: boolean;
};
...
export default function LabSettingsForm({
  ...
  const save = async () => {
    ...
    try {
      ...
      if (embedded) alert('✅ Реквизиты лаборатории сохранены');
      onSaved?.();
    } catch (e: any) {
  ...
  if (loading) {
    return <p style={{ color: COLORS.muted }}>Загрузка...</p>;
  }

  return (
    <div>
      {!embedded && (
        <p style={{ margin: '0 0 16px', color: COLORS.muted, fontSize: 13, lineHeight: 1.4 }}>
      ...
      {embedded && (
        <p style={{ margin: '0 0 16px', color: COLORS.muted, fontSize: 13, lineHeight: 1.4 }}>
          Те же реквизиты, что в Лаборатория → «Реквизиты». Изменения сразу подхватывают новые паспорта.
        </p>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        ...
        <Field
          label="Класс Аэфф"
          value={form.aeff_class || ''}
          onChange={(v) => set('aeff_class', v)}
          placeholder="I класс, не более 370 Бк/кг"
        />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 140px', gap: 12, alignItems: 'start' }}>
        <div style={{ display: 'grid', gap: 12 }}>
          <Field
            ...
            value={form.declaration_concrete || ''}
            onChange={(v) => set('declaration_concrete', v)}
          />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 140px', gap: 12, alignItems: 'start' }}>
        <div style={{ display: 'grid', gap: 12 }}>
          <Field
            label="Номер декларации"
            value={form.declaration_mortar || ''}
            onChange={(v) => set('declaration_mortar', v)}
          />
