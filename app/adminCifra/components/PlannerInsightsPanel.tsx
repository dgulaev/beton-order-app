...
type Props = {
  ...
  uiScale?: number;
  canEdit: boolean;
  /** Компактный вид для боковой колонки (page layout). */
  compact?: boolean;
};
...
export default function PlannerInsightsPanel({
  ...
  const fs = (n: number) => Math.round(n * uiScale * (compact ? 0.92 : 1));
  const sp = (n: number) => Math.round(n * uiScale * (compact ? 0.9 : 1));
  ...
  return (
    <div
      style={volumeCardStyle({
        ...
        padding: compact ? `${sp(10)}px ${sp(12)}px` : `${sp(14)}px ${sp(16)}px`,
        flexShrink: 0,
    ...
    >
      <div
        style={{
          ...
          alignItems: 'center',
          gap: sp(8),
          marginBottom: sp(compact ? 8 : 12),
        }}
      >
        <Brain size={fs(compact ? 18 : 22)} color="#6EE7B7" />
        <div style={{ fontSize: fs(compact ? 15 : 18), fontWeight: 800, color: '#F1F5F9', flex: 1 }}>
          {compact ? 'Подсказки V2' : 'Подсказки V2 · план vs факт'}
        </div>
        <button
          type="button"
        ...
        >
          {[
            ...
            <div
              ...
              style={volumeCardSoftStyle({
                borderRadius: 12,
                padding: compact ? `${sp(8)}px ${sp(10)}px` : `${sp(10)}px ${sp(12)}px`,
              })}
            >
              <div style={{ color: '#94A3B8', fontSize: fs(compact ? 11 : 12), fontWeight: 600 }}>
                {c.label}
              </div>
              <div
                style={{
                  color: '#E2E8F0',
                  fontSize: fs(compact ? 16 : 20),
                  fontWeight: 800,
                  marginTop: 4,
                }}
              >
      ...
      >
        {(data?.tips || []).slice(0, compact ? 2 : undefined).map((t, i) => (
          <div
            key={i}
            style={{
              display: 'flex',
              ...
              fontSize: fs(compact ? 12 : 15),
              fontWeight: 600,
              lineHeight: 1.35,
            }}
          >
            {t.tone === 'warn' ? (
              <AlertTriangle size={fs(compact ? 14 : 16)} style={{ flexShrink: 0, marginTop: 2 }} />
            ) : (
              <CheckCircle2 size={fs(compact ? 14 : 16)} style={{ flexShrink: 0, marginTop: 2 }} />
            )}
            <span>{t.text}</span>
          </div>
        ))}
      ...
      {(data?.risks || []).length > 0 ? (
        <div style={{ marginBottom: sp(compact ? 8 : 12) }}>
          <div style={{ color: '#F87171', fontSize: fs(compact ? 12 : 14), fontWeight: 700, marginBottom: 6 }}>
            Риски
          </div>
          {(data?.risks || []).slice(0, compact ? 3 : 6).map((r, i) => (
            <div key={i} style={{ color: '#FCA5A5', fontSize: fs(compact ? 11 : 13), fontWeight: 600 }}>
              {r.orderId ? `#${r.orderId}` : '—'}
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: sp(8) }}>
        <ModalActionButton
          ...
          label={learning ? 'Учусь…' : compact ? 'Обучение (день)' : 'Обновить обучение (день)'}
          size={compact ? 'sm' : 'lg'}
