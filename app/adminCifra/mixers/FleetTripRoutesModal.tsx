...
export default function FleetTripRoutesModal({
  ...
  const mapRoutes: FleetMapRoute[] = useMemo(() => {
    ...
    // Факт GPS — пунктир для сравнения с планом (отклонения)
    if (actual && planned) {
      list.push({
        id: `${r.tripId}-gps`,
        color: '#F8FAFC',
  ...
  return (
    <>
      ...
      <div
        style={{ position: 'fixed', inset: 0, zIndex: 959, background: 'rgba(0,0,0,0.72)' }}
        onClick={onClose}
      />
        ...
        <div className="fleet-trip-routes-panel">
          ...
          >
            ...
            <div style={{ flex: 1, minWidth: 160 }}>
              <div style={{ fontWeight: 700, color: '#F8FAFC', fontSize: 17 }}>
          ...
          <div style={{ flex: 1, minHeight: 0, display: 'flex' }}>
                ...
                <FleetMap
                  ...
                  emptyMessage={
                    ...
                    (routes.length === 0
                      ...
                      : selectedTripId == null
                        ? 'Выбери рейс в списке справа'
                        : 'Нет координат маршрута')
                  }
                />
              )}

              {/* Легенда: стили линий + цвета рейсов */}
              {!loading && routes.length > 0 && (
                <div
                  style={{
                    position: 'absolute',
            ...
            >
              <div style={{ color: '#94A3B8', fontSize: 12, marginBottom: 4 }}>
                ...
                {routes.length > 0 && (
                  <span style={{ color: '#64748B' }}> · на карте выбранный</span>
                )}
              </div>
              {error && (
