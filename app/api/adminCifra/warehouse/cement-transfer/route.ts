...
async function loadTrips(opts: {
  ...
  if (opts.fromSiloId != null) {
    query = query.eq('cement_write_off_silo_id', opts.fromSiloId);
  }
  ...
  return (data || []).map((row: any) => {
    ...
    return {
      ...
      grade: row.orders?.grade ? String(row.orders.grade) : null,
      deliveryDate: row.orders?.delivery_date ? String(row.orders.delivery_date) : null,
      siloId,
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const rawIds: unknown[] = Array.isArray(body?.mixerIds) ? body.mixerIds : [];
    const mixerIds: number[] = [];
    ...
    for (const id of mixerIds) {
      results.push(await transferOne(id, toSiloId, actorName));
    }
