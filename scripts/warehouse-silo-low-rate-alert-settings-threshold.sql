-- Порог алерта низкого расхода силоса: опциональный p_threshold из system_settings.
-- Без параметра — прежние 5/10 т. Идемпотентно.
...
begin
  ...
  v_threshold := case
    when p_threshold is not null and p_threshold >= 0 then p_threshold
    when p_silo_id = 3 then 10
    else 5
  end;
  ...
  -- Безопасно: current >= -порог (ровно −N ещё не алерт)
